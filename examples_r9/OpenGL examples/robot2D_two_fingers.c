/*
   ...
   This version add another finger segement, drawn at the wrist joint.
*/
#include <stdio.h>
...
void display(void)
{
   ...
   glPushMatrix();  // save the wrist coordinate system
   ...
   glPopMatrix();  // restore the wrist coordinate system

   // draw another finger, but in the wrist cordinate system
   glRotatef( 2*(GLfloat)finger, 0.0, 0.0, 1.0);
   glBegin(GL_LINES);
     glVertex2f( 0.0,  0.0);    // finger line segment
     glVertex2f( 1.0,  0.0);
   glEnd();
...
/*
   Notice that every one of the above five line segments is drawn along the
