export default function rasterize(model, ls, vp)
{
    ...
    for(x = Math.trunc(x0); x < Math.trunc(x1); x += 1, y += m)
    {
        ...
        if(rasterize.doAntiAliasing)
        {
            ...
            const weight = 1/(y - yLow);

            rL = (1-wieght) * r + weight * (bg.getRed()/255);
            gL = (1-wieght) * g + weight * (bg.getGreen()/255);
            bL = (1-wieght) * b + weight * (bg.getBlue()/255);

            rH = weight * r + (1-weight) * (bg.getRed()/255);
