   ...
   */
   constructor(xMin=-1, xMax=1, yMin=-1, yMax=1, xMarks=5, yMarks=5, cX=Color.white, cY=Color.white, z=0)
   {
      ...
      this.addVertex(new Vertex(xMin, 0, z),
                     new Vertex(xMax, 0, z));
      this.addPrimitive(LineSegment.buildVertexColor(0, 1, 0)); // use color cX1, 0));

      // y-axis
      this.addVertex(new Vertex(0, yMin, z),
                     new Vertex(0, yMax, z));
      this.addPrimitive(LineSegment.buildVertexColor(2, 3, 1)); // use color cY
      ...
      for(let x = xMin; x <= xMax; x += xDelta)
      {
         ...
         this.addPrimitive(LineSegment.buildVertexColor(index+0, index+1, 0)); // use color cXex+1]));
         index += 2;
      }
      ...
      for(let y = yMin; y <= yMax; y += yDelta)
      {
         ...
         this.addPrimitive(LineSegment.buildVertexColor(index+0, index+1, 1)); // use color cYex+1]));
         index += 2;
      }
   }
}//Axes2D
