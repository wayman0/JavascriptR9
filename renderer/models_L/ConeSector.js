...
export default class ConeSector extends Model
{
    ...
    constructor(rad=1, height=1, top = height, t1=Math.PI/2, t2=Math.PI/2, n=15, k=8)
    {
        ...
        if( typeof rad != "number" || typeof height != "number" || typeof top != "number" ||
        ...
        for(let x = 0; x < n; x += 1)
            for(let y = 0; y < k-1; y += 1)
                this.addPrimitive(LineSegment.buildVertex(vertInd[x][y], vertInd[x][y]+1));
        ...
        for(let x = 0; x < k; x += 1)
        {
            ...
            for(let y = 0; y < n-1; y += 1)
                this.addPrimitive(LineSegment.buildVertex(vertInd[y][x], vertInd[y+1][x]));
        }
